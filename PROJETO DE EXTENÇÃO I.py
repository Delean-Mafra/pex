...
def calcular_hash(arquivo):
    """Calcula o hash do conteúdo do arquivo baseado no tipo."""
    ...
    try:
        ...
        # Em caso de erro, usar hash do arquivo completo
        with open(arquivo, 'rb') as f:
            for chunk in iter(lambda: f.read(4096), b""):
                hash_sha256.update(chunk)
...
def verificar_duplicados(caminho_pasta):
    """Verifica e remove arquivos duplicados na pasta especificada."""
    arquivos_verificados = {}
    log_exclusao = []
    
    # Verifica se a pasta existe
    if not os.path.exists(caminho_pasta):
        return False, "Pasta não encontrada.", None
                ...
                try:
                    ...
                    if hash_arquivo in arquivos_verificados:
                        ...
                        try:
                            ...
                            log_exclusao.append(f"Erro ao excluir {caminho_completo}: {str(e)}")
                    else:
...
def process():
    """Processa a exclusão de arquivos duplicados."""
    try:
...
def select_folder():
    """Abre um diálogo nativo para selecionar pasta (executa no host)."""
    try:
        ...
        return jsonify({'success': False, 'message': f'Erro ao abrir seletor: {str(e)}'})
